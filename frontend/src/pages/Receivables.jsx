...
import { Modal, PageHeader, Button, FormField, Card } from '../components';
...
export default function Receivables() {
    ...
    const handleOpenPayment = (account) => {
        ...
        setPaymentForm(prev => ({ ...prev, monto: '', descripcion: '', medio_pago_id: '' }));
        setShowModal(true);
    };
    ...
    const handlePaymentSubmit = async (e) => {
        e.preventDefault();
        if (!paymentForm.monto || !paymentForm.medio_pago_id) {
            toast.warning("Completa el monto y medio de pago");
            return;
        }
        ...
        try {
            const payload = {
                ...
                medio_pago_id: parseInt(paymentForm.medio_pago_id)
            };
            ...
            console.error(error);
            toast.error("Error al procesar abono");
        }
    };
    ...
    const handlePaySpecificOrder = async () => {
        if (!selectedOrder || !paymentForm.medio_pago_id) {
            toast.warning("Selecciona un pedido y un medio de pago");
            return;
        }
            ...
            >
                ...
                <form onSubmit={handlePaymentSubmit} className="flex flex-col gap-4">
                    ...
                    <FormField label="Medio de Pago" icon={<CreditCard size={16} />} required>
                        <select
                            value={paymentForm.medio_pago_id}
                            onChange={e => setPaymentForm({ ...paymentForm, medio_pago_id: e.target.value })}
                            required
                        >
                    ...
                    <FormField label="Fecha del Recibo" icon={<Calendar size={16} />} required>
                        <DateInput value={paymentForm.fecha} onChange={v => setPaymentForm({ ...paymentForm, fecha: v })} required height={42} />
                    </FormField>
                                ...
                                <form className="flex flex-col gap-4">
                                    <div className="stats-grid">
                                        ...
                                        <FormField label="Fecha Pago" icon={<Calendar size={16} />}>
                                            <DateInput value={paymentForm.fecha} onChange={v => setPaymentForm({ ...paymentForm, fecha: v })} height={42} />
                                        </FormField>
                                    </div>
