...
export default function Transfers() {
    ...
    const loadData = async () => {
        setLoading(true);
        const [transfersRes, methodsRes, balancesRes] = await Promise.allSettled([
    ...
    const handleSubmit = async (e) => {
        ...
        if (Number(form.origen_id) === Number(form.destino_id)) {
            toast.warning("El origen y destino no pueden ser iguales");
            return;
        }
        ...
        try {
            const payload = {
                ...
                fecha: (form.fecha ? form.fecha + 'T00:00:00' : new Date().toISOString()),
            ...
            console.error(error);
            toast.error("Error al guardar transferencia");
        }
    };
                        ...
                        {loading ? (
                            ...
                            transfers.map(t => (
                                <tr key={t.id} style={{ borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                                    ...
                                    <td className="p-3 text-sm">{formatDate(t.fecha)}</td>
